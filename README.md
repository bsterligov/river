...
[RTK](https://github.com/rtk-ai/rtk) was used to compress Bash tool output — 210K additional input tokens avoided (~$0.63). Since week 1, a second layer (`river-index`) was added to intercept `Read` tool calls on source files and return compact summaries — see the Token optimization section below for details.
