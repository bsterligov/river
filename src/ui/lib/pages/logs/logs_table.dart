...
List<double> computeColumnWidths(
  ...
  final gaps = (columns.length - 1) * AppLayout.gapM;
  final usable = (available - _settingsReserved - gaps).clamp(0.0, double.infinity);
  ...
  for (final col in columns) {
    ...
    double dataW;
    if (col.fixedSample != null) {
      dataW = _measureText(col.fixedSample!, AppText.mono);
    } else {
      dataW = sample.fold(0.0, (max, row) {
        final w = _measureText(col.getValue(row), AppText.mono);
        return w > max ? w : max;
      });
    }
    final w = (labelW > dataW ? labelW : dataW) + _colPad;
    natural.add(w);
    fixedTotal += w;
  }

  if (fixedTotal >= usable) {
    final scale = usable / fixedTotal;
    return [
  ...
  final stretchWidth = stretchCount > 0
  ...
  // Guard: if sub-pixel drift causes the total to exceed usable, shave from stretchy columns.
  final total = result.fold(0.0, (s, w) => s + w);
  if (total > usable && stretchCount > 0) {
    final excess = total - usable;
    final adjustedStretch = (stretchWidth - excess / stretchCount).clamp(60.0, double.infinity);
    return [
      for (int i = 0; i < columns.length; i++)
        columns[i].stretchy == true ? adjustedStretch : natural[i],
    ];
  }
  return result;
}
