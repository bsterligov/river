...
- **Metrics page module:** `lib/pages/metrics/` with barrel `metrics.dart`; `MetricsController` (ChangeNotifier) owns only metric names and loading/error state; `_GraphTabState` owns its own `_selected` set, `_series` map, and loading state — graph selection is fully decoupled from the All Metrics list (no shared selection); `TabController` held by `_ShellState` (length: 2, vsync: `TickerProviderStateMixin`)
- **Graph tab auto-refresh:** series reload automatically when a metric is added/removed or when `TimeRangeController` fires; no manual Refresh button
