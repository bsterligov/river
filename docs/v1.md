...
flowchart TD
    ...
    SA --> IA["/issue-create → spec PR"]
    SB --> IB["/issue-create → spec PR"]
...
- All phases in a wave spawn in parallel (one subagent per phase, each running `/dev-spec` in an isolated git worktree).
